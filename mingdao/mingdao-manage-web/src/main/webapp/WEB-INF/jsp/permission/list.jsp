							...
							<c:forEach items="${datas.datas}" var="role">
								<tr>
									...
									<td>
										...
										<a class="btn btn-xs btn-danger" href="deletePermission/${role.id }" title="删除">
							...
							<table style="width: 100%;">
								...
								<tr>
									<td style="vertical-align: top;">
										...
										<a href="<%=request.getContextPath() %>/permission/permissions" style="color:#FFF;text-decoration:none;" class="btn btn-info fa fa-refresh" title="刷新列表"></a>
									</td>
									<td style="vertical-align: top;">
					...
					<div id="fbox_grid-table" class="searchFilter" style="overflow:auto">
						<table class="group ui-widget ui-widget-content ui-search-table" style="border:0px none;">
							...
							<tr>
								...
								<td class="data"><input type="text" id="search_PermissionName" name="search_PermissionName" role="textbox"
							...
							<tr>
								...
								<td class="data"><input type="text"  id="search_PermissionCode" name="search_PermissionCode" role="textbox"
														class="input-elm ui-widget-content ui-corner-all" style="width: 96%;">
								</td>
							</tr>
							</tbody>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="ui-jqdialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix" id="searchhdfbox_grid-table_add"
				 style="cursor: move;">
				<div class="widget-header">
					<span class="ui-jqdialog-title" id="modalTitle" style="float: left;">新增权限</span>
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">
						&times;
					</button>
				</div>
					...
					<div class="row">
						<div class="col-xs-12">
							<div class="form-group">
								<label class="col-sm-3 control-label no-padding-right" for="permissionCode">权限编码: </label>

								<div class="col-sm-9">
									<input id="id" placeholder="id" class="col-xs-10 col-sm-5" type="hidden">
									<input id="permissionCode" placeholder="permissionCode" class="col-xs-10 col-sm-5" type="text">
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-9">
									<input id="permissionName" placeholder="permissionName" class="col-xs-10 col-sm-5" type="text">
								</div>
							</div>

							<div class="form-group">
								<label class="col-sm-3 control-label no-padding-right" for="permissionMemo">  权限备注: </label>

								<div class="col-sm-9">
									<input id="permissionMemo" placeholder="permissionMemo" class="col-xs-10 col-sm-5" type="text">
								</div>
							</div>
						</div>
					</div>
...
<script type="text/javascript">
	...
	function editPermission(userId,obj){
		...
		$.ajax({
			...
			success: function (data) {
				...
				$("#id").val(user.id);
				$("#permissionCode").val(user.permissionCode);
				$("#permissionName").val(user.permissionName);
				$("#permissionMemo").val(user.permissionMemo);
			},
			fail: function (err) {
				console.log(err)
			}
	...
	$("#fbox_grid-table_search").click(function(){
		...
		paramsName.name="search_PermissionName";
		paramsName.val=$("#search_PermissionName").val();

		var paramsCode = new Object();
		paramsCode.name="search_PermissionCode";
		paramsCode.val=$("#search_PermissionCode").val();

		var paramsArr = [paramsName,paramsCode];
		submitForm("<%=request.getContextPath() %>/permission/permissions",paramsArr);
	});
	...
	$("#fbox_grid-table_add").click(function(){
		...
		var permissionMemo = $("#permissionMemo").val();
		var id = $("#id").val();
		$.ajax({
			type: 'POST',
			url: "<%=request.getContextPath() %>/permission/addPermission",
