				...
				<div>
					<div id="fbox_grid-table" class="searchFilter" style="overflow:auto">
						<table class="group ui-widget ui-widget-content ui-search-table" style="border:0px none;">
							...
							<tr>
								<td class="first"></td>
								<td class="columns"><label>门店名称：</label></td>
								<td class="data">
									<input type="text" id="search_StoreName" name="search_StoreName" role="textbox"
														class="input-elm ui-widget-content ui-corner-all" style="width: 96%;">
								</td>

								<td class="first"></td>
								<td class="columns"><label>门店编号：</label></td>
							...
							<table id="sample-table-1" class="table table-striped table-bordered table-hover">
								...
								<c:forEach items="${datas.datas}" var="userInfo">
									<tr>
										...
										<td>${userInfo.tel1}</td>
										<td><input name="checked" type="radio" value="${userInfo.id},${userInfo.name}" /></td>
									</tr>
								</c:forEach>
								</tbody>
							</table>
										...
										<tr>
											<td style="vertical-align: top;">
												<c:if test="${datas.total > 0}">
														...
														<jsp:param value="refStores" name="url"/>
													</jsp:include>
												</c:if>
											</td>
										</tr>
...
<script src="<%=request.getContextPath() %>/resources/ace/assets/js/jquery.dataTables.bootstrap.js"></script>
	...
	<script type="text/javascript">
		    function getSelecOption(){
				...
				parent.document.getElementById("storeId").value = array[0];
				parent.document.getElementById("storeName").value = array[1];
				parent.document.getElementById("storeList").style.display="none";
			}
			...
			$("#fbox_grid-table_search").click(function(){
				...
				submitForm("<%=request.getContextPath() %>/storeBaseSer/refStores",paramsArr);
	    	});
	</script>
